...
%   lr = logspace(-1, -4, 20) ;
% end
lr = [1e-1*ones(1,80), 1e-2*ones(1, 80), 1e-3*ones(1, 40)];
...
% --------------------------------------------------------------------
function net = add_scn_block(net, opts, id, h, w, in, out, stride, pad)
% --------------------------------------------------------------------
...
lambda = 0*1e-4;
lambda2 = 0*1e-6;
%  sc = sqrt(2 / (ksize^2*ndim_in)) ;
%  init_weights = (randn(ksize, ksize, ndim_in,  ndim_out, 'single'))*sc;
...
% net = insertSCBlock(net, 11, 3, 48*K, 4, 1);   %56*56
...
% net = add_block(net, opts, id, h, w, in, out, stride, pad)
% net = add_block(net, opts, '2', 5, 5, 48, 256, 1, 2) ;
net = insertSCBlock(net, 3, 96, 96/Kd, 2, 1);   %14x14
net = insertSCBlock(net, 3, 96/Kd, 128, 1, 1);   %14x14
...
%                            'stride', 2, ...
%                            'pad', 0) ;
net = insertSCBlock(net, 3, 128, 192/Kd, 1, 1);   %14x14
net = insertSCBlock(net, 3, 192/Kd, 192, 1, 1);   %14x14

net = insertSCBlock(net, 3, 192, 128/Kd, 2, 1);   %14x14
net = insertSCBlock(net, 3, 128/Kd, 128, 1, 1);   %14x14
% net = insertSCBlock(net, 3, 128*K, 192*K, 1, 1);
% net = add_block(net, opts, '3', 3, 3, 256, 384, 1, 1) ;
% net = insertSCBlock(net, 3, 192*K, 192*K, 1, 1);
% net = add_block(net, opts, '4', 3, 3, 192, 384, 1, 1) ;
% net = insertSCBlock(net, 3, 192*K, 128*K, 1, 1);
...
net = add_block(net, opts, '6', 7, 7, 128, 4096, 1, 0) ;
