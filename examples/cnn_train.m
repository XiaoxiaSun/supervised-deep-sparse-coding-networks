...
for t=1:params.batchSize:numel(subset)
    ...
    for s=1:params.numSubBatches
        ...
        [res, net] = vl_simplenn(net, im, dzdy, res, ...
            'accumulate', s ~= 1, ...
            'mode', evalMode, ...
...
for l=numel(net.layers):-1:1
    for j=numel(res(l).dzdw):-1:1
        ...
        if j == 3 && strcmp(net.layers{l}.type, 'bnorm')
            ...
            %     force lambda>0
            %             threshold = 1e-5;
            if(strcmp('sc_layer', net.layers{l}.type) && j==2)
                net.layers{l}.weights{j}((net.layers{l}.weights{j})<0) = 0;   %%% abs enabled
            end
        ...
        if params.plotDiagnostics
            ...
            %       res(l).stats.error_rate = error_rate;
            %       res(l).stats.nonzero_rate = nonzero_rate;
