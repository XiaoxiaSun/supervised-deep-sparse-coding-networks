function [res, net] = vl_simplenn(net, x, dzdy, res, varargin)
...
for i=1:n
    ...
    switch l.type
        case 'sc_layer'
            [res(i+1).x] = forward_sparse_coding_layer(res(i).x, l.weights{1}, l.weights{2},  l.stride, testMode);
            
        case 'gnorm'
            [ res(i+1).x] = gnorm( res(i).x );
        case 'shrinkage'
            res(i+1).x = forward_shrinkage(res(i).x, l.weights{1});
            
        case 'polarized_pool'
            [res(i+1).x, res(i+1).mask] = mexPolarizedPooling_3D_forward(res(i).x, l.pool, l.stride, l.kstride);
        case 'conv'
        ...
        case 'incoherentloss'
            res(i+1).x = incoherentloss(res(i).x);
            
        case 'abs'
            res(i+1).x = vl_nnabs(res(i).x,[]) ;
...
if doder
    ...
    for i=n:-1:backPropLim
        ...
        switch l.type
            case 'sc_layer'
                [res(i).dzdx, dzdw{1}, dzdw{2}] = backward_sparse_coding_layer(res(i).x, res(i+1).x, l.weights{1}, l.weights{2}, ...
                                                                                                                                                                    res(i+1).dzdx,  l.stride, opts.numSlice);  % set dropout rate to be 0 in testing
                
            case 'euclideanloss'
                res(i).dzdx = euclideanloss(res(i).x, l.class, res(i+1).dzdx);
            case 'incoherentloss'
                res(i).dzdx = incoherentloss(res(i).x, res(i+1).dzdx);
            case 'gnorm'
                [ ~, res(i).dzdx ] = gnorm( res(i).x,  res(i+1).dzdx);
                
            case 'shrinkage'
                [dzdw{1}, res(i).dzdx] = backward_shrinkage(res(i).x, l.weights{1}, res(i+1).dzdx);
                
                
            case 'polarized_pool'
                res(i).dzdx = mexPolarizedPooling_3D_backward(res(i+1).dzdx, res(i+1).mask, l.pool, l.stride, l.kstride, size(res(i).x, 1), size(res(i).x, 2));
                %             [res(i).dzdx, dzdw{1}] = backward_sparse_coding_layer(res(i).x, res(i+1).x, l.weights{1}, res(i+1).dzdx, l.K, l.stride);
                %             wait(gpuDevice)
                
            case 'abs'
                res(i).dzdx = vl_nnabs(res(i).x, res(i+1).dzdx);
        ...
        switch l.type
            case {'conv', 'convt', 'bnorm', 'sc_layer'}
                if ~opts.accumulate
                    res(i).dzdw = dzdw ;
                else
                    for j=1:numel(dzdw)
